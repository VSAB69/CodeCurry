    ...
    <PageTransition>
      <div 
        ...
        className={`min-h-screen transition-colors duration-1000 ${
          isNightMode ? 'bg-[#050505] text-white' : 'bg-[#0A0A0A] text-white'
        ...
        <section className="py-32 relative z-20 bg-black">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="text-center mb-16 reveal-section">
              ...
              <p className="text-gray-400 text-xl max-w-2xl mx-auto">Navigate through our state-of-the-art facilities and vibrant spaces.</p>
            </div>

            <div className="relative w-full aspect-[16/9] md:aspect-[21/9] bg-white/5 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-sm reveal-section group">
              <div className="absolute inset-0 bg-[url('https://www.collegebatch.com/static/clg-gallery/bms-college-of-engineering-bangalore-361119.webp')] bg-cover bg-center opacity-40 group-hover:opacity-30 transition-opacity duration-700" />
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent" />
              
              {/* Map Hotspots */}
              <div className="absolute inset-0">
                {[
                  { id: 'academic', label: 'Academic Blocks', x: '30%', y: '40%', icon: Building2, color: 'blue' },
                  { id: 'library', label: 'Central Library', x: '50%', y: '30%', icon: BookOpen, color: 'purple' },
                  { id: 'labs', label: 'Innovation Labs', x: '70%', y: '45%', icon: FlaskConical, color: 'emerald' },
                  { id: 'hostels', label: 'Student Hostels', x: '20%', y: '70%', icon: Home, color: 'orange' },
                  { id: 'sports', label: 'Sports Complex', x: '80%', y: '65%', icon: Dumbbell, color: 'red' },
                  { id: 'cafeteria', label: 'Cafeteria', x: '45%', y: '60%', icon: Coffee, color: 'yellow' },
                  { id: 'hospital', label: 'BMS Hospital', x: '60%', y: '80%', icon: HeartPulse, color: 'pink' },
                ].map((spot, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    <div className={`w-12 h-12 -ml-6 -mt-6 rounded-full bg-${spot.color}-500/20 border border-${spot.color}-500/50 flex items-center justify-center backdrop-blur-md relative`}>
                      <spot.icon className={`w-5 h-5 text-${spot.color}-400`} />
                      <div className={`absolute inset-0 rounded-full border border-${spot.color}-400 animate-ping opacity-20`} />
                    </div>
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 opacity-0 group-hover/spot:opacity-100 transition-opacity whitespace-nowrap">
                      <div className="px-3 py-1 rounded-lg bg-black/80 border border-white/10 text-sm font-medium backdrop-blur-md">
                        {spot.label}
                      </div>
                    </div>
                  </motion.div>
